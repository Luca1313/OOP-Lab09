...
public final class LambdaUtilities {
    ...
    public static <T> List<Optional<T>> optFilter(final List<T> list, final Predicate<T> pre) {
        final List<Optional<T>> retList = new ArrayList<>(list.size());
        list.forEach(t -> {
            Optional<T> option = Optional.of(t);
            retList.add(option.filter(pre));
        });
        /*
    ...
    public static <R, T> Map<R, Set<T>> group(final List<T> list, final Function<T, R> op) {
        ...
        list.forEach(t -> {
            ...
            newSet.add(t);
            retMap.merge(tmpR, newSet, (t1, t2) -> {
                t1.addAll(t2);
                return t1;
            });
        });
